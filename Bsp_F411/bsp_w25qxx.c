...
{
    HAL_GPIO_WritePin(SPI2_CS_GPIO_Port, SPI2_CS_Pin, Status);
}
...
void W25QXX_Init(void)
{
    ...
    if(W25QXX_TYPE == W25Q256)  //SPI Flash为W25Q256
    {
        ...
        if((temp_SR&0x01) == 0)  //不是4字节模式，则设置4字节模式
        {
            ...
            SPI2_ReadWriteByte(W25X_Enable4ByteAddr);  //4字节地址模式指令
            W25QXX_CS(GPIO_PIN_SET);  //SPI Flash不选中
        }
    }
}
...
uint16_t W25QXX_ReadID(void)
{
    ...
    SPI2_ReadWriteByte(W25X_ManufactDeviceID);  //读取设备ID
...
uint8_t W25QXX_ReadSR(uint8_t Regno)
{
    ...
    W25QXX_CS(GPIO_PIN_RESET);
    SPI2_ReadWriteByte(Command);  //发送指令
...
void W25QXX_Write_Page(uint32_t WriteAddr, uint8_t* pBuffer, uint16_t NumByteToWrite)
{
    W25QXX_Write_Enable();
    W25QXX_CS(GPIO_PIN_RESET);
    SPI2_ReadWriteByte(W25X_PageProgram);  //写页指令
    ...
    for(uint16_t nCount=0; nCount<NumByteToWrite; nCount++)  //循环写入
        SPI2_ReadWriteByte(pBuffer[nCount]);
    W25QXX_CS(GPIO_PIN_SET);
    W25QXX_Wait_Busy();
}
...
void W25QXX_Read(uint32_t ReadAddr, uint8_t* pBuffer, uint16_t NumByteToRead)
{
    ...
    for(uint16_t nCount=0; nCount<NumByteToRead; nCount++)  //循环读数据
        pBuffer[nCount] = SPI2_ReadWriteByte(0xFF);
    W25QXX_CS(GPIO_PIN_SET);
}
...
void W25QXX_Erase_Chip(void)
{
    ...
    W25QXX_Wait_Busy();
    W25QXX_CS(GPIO_PIN_RESET);
    SPI2_ReadWriteByte(W25X_ChipErase);  //全盘擦除指令
    W25QXX_CS(GPIO_PIN_SET);
    W25QXX_Wait_Busy();
}
...
{
    W25QXX_CS(GPIO_PIN_RESET);
    SPI2_ReadWriteByte(W25X_Sleep);  //休眠指令
    W25QXX_CS(GPIO_PIN_SET);
    YMX_Delay(5);
}
...
{
    W25QXX_CS(GPIO_PIN_RESET);
    SPI2_ReadWriteByte(W25X_WakeUp);  //唤醒指令
    W25QXX_CS(GPIO_PIN_SET);
    YMX_Delay(5);
}
